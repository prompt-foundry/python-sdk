...
async def test_basic(sync: bool, client: PromptFoundrySdk, async_client: AsyncPromptFoundrySdk) -> None:
...
async def test_data_missing_event(
    sync: bool, client: PromptFoundrySdk, async_client: AsyncPromptFoundrySdk
) -> None:
    def body() -> Iterator[bytes]:
        yield b'data: {"foo":true}\n'
        yield b"\n"
...
async def test_special_new_line_character(
    ...
    async_client: AsyncPromptFoundrySdk,
) -> None:
    def body() -> Iterator[bytes]:
