...
from prompt_foundry_python_sdk import BaseModel, PromptFoundry, AsyncPromptFoundry
from prompt_foundry_python_sdk._response import (
    APIResponse,
    BaseAPIResponse,
    AsyncAPIResponse,
    BinaryAPIResponse,
...
from prompt_foundry_python_sdk._streaming import Stream
from prompt_foundry_python_sdk._base_client import FinalRequestOptions
...
def test_extract_response_type_direct_class_missing_type_arg() -> None:
    with pytest.raises(
        RuntimeError,
        match="Expected type <class 'prompt_foundry_python_sdk._response.AsyncAPIResponse'> to have a type argument at index 0 but it did not",
    ):
...
def test_response_parse_mismatched_basemodel(client: PromptFoundry) -> None:
    ...
    with pytest.raises(
        TypeError,
        match="Pydantic models must subclass our base model type, e.g. `from prompt_foundry_python_sdk import BaseModel`",
    ):
...
def test_response_parse_bool(client: PromptFoundry, content: str, expected: bool) -> None:
    response = APIResponse(
...
async def test_async_response_parse_bool(client: AsyncPromptFoundry, content: str, expected: bool) -> None:
    response = AsyncAPIResponse(
        raw=httpx.Response(200, content=content),
        client=client,
        stream=False,
...
def test_response_parse_expect_model_union_non_json_content(client: PromptFoundry) -> None:
    response = APIResponse(
...
async def test_async_response_parse_expect_model_union_non_json_content(async_client: AsyncPromptFoundry) -> None:
    response = AsyncAPIResponse(
        raw=httpx.Response(200, content=b"foo", headers={"Content-Type": "application/text"}),
        client=async_client,
        stream=False,
