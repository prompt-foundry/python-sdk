...
from prompt_foundry_python_sdk import PromptFoundry, AsyncPromptFoundry, DefaultAioHttpClient
from prompt_foundry_python_sdk._utils import is_dict
...
logging.getLogger("prompt_foundry_python_sdk").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[PromptFoundry]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with PromptFoundry(base_url=base_url, api_key=api_key, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncPromptFoundry]:
